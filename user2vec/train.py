...
def train_it(model, train_x, val_x, neg_samples, initial_lr, epochs, device):        
    # ipdb.set_trace() 
    model.to(device)
    # val_x_ = torch.from_numpy(val_x).long().to(device)
...
def train(path, output, epochs=20, initial_lr=0.001):
    ...
    for j, user in enumerate(user_data):
        ...
        model = train_it(model, train_x, val_x, neg_samples, initial_lr, epochs, "cpu")
